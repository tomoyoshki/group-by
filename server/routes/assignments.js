...
module.exports = function (router) {
    ...
    assignmentsRoute.post(async function(req, res) {
        ...
        try {
            ...
            // update instrucotr
            let cur_instructor = await user.findOne({_id: req.body.instructor_id}, {}).catch(err => {})
            if (!cur_instructor.created_assignment_ids.includes(assignments._id)) {
                cur_instructor.created_assignment_ids.push(assignments._id)
            }
            await cur_instructor.save().catch(err => {})
